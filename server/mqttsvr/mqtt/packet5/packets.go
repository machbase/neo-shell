...
import (
	...
	"github.com/machbase/neo-shell/server/mqttsvr/mqtt/mqtterr"
)
...
func ReadPacket(r io.Reader, maxMessageSizeLimit int) (*ControlPacket, int, error) {
	...
	case AUTH:
		...
		cp.Content = &Auth{Properties: &Properties{}}
	default:
		return nil, nbytes, fmt.Errorf("unknown packet type %d requested", pt)
	}
	...
	if maxMessageSizeLimit > 0 && cp.remainingLength > maxMessageSizeLimit {
		return nil, nbytes, mqtterr.MaxMessageSizeExceededError(cp.remainingLength, maxMessageSizeLimit)
	}
	...
	if err != nil {
		return nil, nbytes + int(n), err
	}

	if n != int64(cp.remainingLength) {
		return nil, nbytes + int(n), fmt.Errorf("failed to read packet, expected %d bytes, read %d", cp.remainingLength, n)
	}
	err = cp.Content.Unpack(&content)

	return cp, nbytes + int(n), err
}
